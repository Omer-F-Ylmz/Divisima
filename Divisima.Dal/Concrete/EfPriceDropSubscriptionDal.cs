...
{
    public class EfPriceDropSubscriptionDal : EfEntityRepositoryBase<PriceDropSubscription, DivisimaDbContext>, IPriceDropSubscriptionDal
    {
        // Açıklayıcı yorum: DbContext'i base'e ilet (EfEntityRepositoryBase parametresiz ctor'a sahip DEĞİL -> zorunlu).
        public EfPriceDropSubscriptionDal(DivisimaDbContext context) : base(context)
        {
        }
        ...
        {
            var affected = await Context.Set<PriceDropSubscription>()
        ...
        {
            await Context.Set<PriceDropSubscription>()
                .Where(x => x.id == id)
