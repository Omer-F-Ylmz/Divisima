...
- **Anormallik/alerting:** `severity == "Critical"` olaylarda `NotifyAdminsAsync` çağrılır (`SecurityEventManager.cs`) ve SignalR `"admins"` grubuna yayın yapılır. **BUGÜN BU GRUP BOŞTUR**: `NotificationHub.JoinAdminGroup()` çağıranı yoktur (ölçüldü: istemci tarafında SignalR 0 geçiş) — bu SignalR dalı bir insana ULAŞMAZ. **MON-1:** SignalR dalı hâlâ boş gruba yayın yapar, ama artık AYRI bir mail okuyucusu var — Hangfire `kritik-olay-alarm` işi 5 dakikada bir yeni `Critical` olayları (yalnız tip + sayı + id; IP/müşteri/ayrıntı taşımadan) `Monitoring:AlarmEmail` adresine gönderir (`ops/monitoring.md`).
