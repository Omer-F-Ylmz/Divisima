...
0b. **LAUNCH-FIX FAZI SURUYOR.** Bes dalga planlandi: A ilk musteri zinciri, B operasyon
   ...
   **SIRADAKI: D-SEMA (YALNIZ OLCUM, kod yazilmaz)** - kullanici karari. Dalga D'nin kalan
   kalemleri bu turdan SONRA, cunku hepsi VERITABANI uzerinde olcum yapiyor ve hangi semanin
   gercek oldugu belli degilken olcum YANILTICI olur. Kapsam: (1) 01_schema.sql <-> EF
   semasi farki tablo tablo, (2) depoda hangisi URETIM icin belgelenmis + acilista migrate
   etmemek bilincli mi, (3) hangisi DOGRU (5-10 FK ornegi, entity kullanimiyla uyum),
   (4) bugune kadarki olcumlerin hangi yolda yapildigi ve diger yolda degisecek olanlar,
   (5) uc secenek (tek kaynak EF / tek kaynak sema dosyasi / ikisi + esitlik pini).
