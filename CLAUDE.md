...
- **SUPHELI #22** - idempotency filtresinin anahtari GOVDEYE bagli degil ve `userScope` daima
  `"anon"`. Bugun SOMURULEMEZ: storefront `Idempotency-Key` basligini HIC gondermiyor (asil
  mekanizma govdedeki `request_id` ve o calisiyor). Risk gelecekteki API istemcilerinedir.
- **GUVENLIK DALGASI 2 / #1 + #2** - misafir checkout enumeration (kayitli e-posta 409,
  kayitsiz 201) ve ayni uctan anonim cop COD siparisi. **KARAR VERILDI: launch sonrasi** -
  409 hesap ele gecirmeyi ENGELLIYOR ve kaldirmak daha buyuk bir riski acar; 10/dk/IP yeterli
  hafifletme (olculdu: 11. istek 429).
...
#22 (LAUNCH SONRASI - bugun istemci kullanmiyor, GUVENLIK DALGASI 2 bulgusu).**
...
22. **IDEMPOTENCY FILTRESININ ANAHTARI GOVDEYE BAGLI DEGIL - AYNI ANAHTARLA FARKLI BIR ISTEK
   SESSIZCE DUSURULUR.** (GUVENLIK DALGASI 2 / B4'te olculdu; kullanici karari: **duzeltme
   LAUNCH SONRASI**, bugun istemci kullanmiyor.)
